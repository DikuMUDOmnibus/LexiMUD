...
struct header_data_type {
	SInt32	next_block;		//	if header block, link to next block
	SInt32	from;			//	idnum of the mail's sender
	SInt32	to;				//	idnum of mail's recipient
...
struct header_block_type_d {
	SInt32	block_type;		/* is this a header or data block?	*/
...
struct data_block_type_d {
	SInt32	block_type;		/*	-1 if header block, -2 if last data block
...
struct mail_index_type_d {
   SInt32 recipient;			/* who is this mail for?	*/
