...
void sedit_setup_existing(DescriptorData *d, RNum rmob_num);
...
void sedit_add_to_type_list(struct shop_buy_data **list, struct shop_buy_data *new_t);
...
void sedit_modify_string(char **str, char *new_t);
void sedit_add_to_int_list(int **list, int new_t);
...
void sedit_setup_existing(DescriptorData *d, RNum rshop_num) { 
	/*
...
void sedit_remove_from_type_list(struct shop_buy_data **list, int num) {
	...
	if (BUY_WORD((*list)[num]))	free(BUY_WORD((*list)[num]));
	FREE(*list);
	*list = nlist;
}
...
void free_shop_strings(struct shop_data *shop) {
	if (S_NOITEM1(shop)) {	free(S_NOITEM1(shop));	S_NOITEM1(shop) = NULL;	}
	if (S_NOITEM2(shop)) {	free(S_NOITEM2(shop));	S_NOITEM2(shop) = NULL;	}
	if (S_NOCASH1(shop)) {	free(S_NOCASH1(shop));	S_NOCASH1(shop) = NULL;	}
	if (S_NOCASH2(shop)) {	free(S_NOCASH2(shop));	S_NOCASH2(shop) = NULL;	}
	if (S_NOBUY(shop)) {	free(S_NOBUY(shop));	S_NOBUY(shop) = NULL;	}
	if (S_BUY(shop)) {		free(S_BUY(shop));		S_BUY(shop) = NULL;		}
	if (S_SELL(shop)) {		free(S_SELL(shop));		S_SELL(shop) = NULL;	}
}
...
void sedit_save_to_disk(int zone_num) { 
	...
	FILE *shop_file;
	char *buf, *fname = get_buffer(64);
	struct shop_data *shop;

	zone = zone_table[zone_num].number;
	top = zone_table[zone_num].top;

	sprintf(fname, SHP_PREFIX "%i.new", zone);
...
void sedit_products_menu(DescriptorData *d) {
	...
	for(i=0;  S_PRODUCT(shop, i) != -1; i++) {
		sprintf(buf, "%2d - [`c%5d`n] - `y%s`n\r\n",  
				i, obj_index[S_PRODUCT(shop, i)].vnum,
				SSData(((ObjData *)obj_index[S_PRODUCT(shop, i)].proto)->shortDesc));
...
void sedit_compact_rooms_menu(DescriptorData *d) {
	...
	for(i=0;  S_ROOM(shop, i) != -1; i++) {
		sprintf(buf, "%2d - [`c%5d`n]  | %s",  
				i, S_ROOM(shop, i), !(++count % 5) ? "\r\n" : "");
		send_to_char(buf, d->character);
	}
	release_buffer(buf);
	send_to_char("\r\n"
...
void sedit_rooms_menu(DescriptorData *d) {
	...
	for(i=0;  S_ROOM(shop, i) != -1; i++) {
		sprintf(buf,  "%2d - [`c%5d`n] - `y%s`n\r\n",  
				i, S_ROOM(shop, i), world[real_room(S_ROOM(shop, i))].GetName("<ERROR>"));
		send_to_char(buf, d->character);
	}
	release_buffer(buf);
	send_to_char("\r\n"
...
void sedit_namelist_menu(DescriptorData *d) {
	...
	for(i=0;  S_BUYTYPE(shop, i) != -1; i++) {
		sprintf(buf, "%2d - `c%15s`n - `y%s`n\r\n",
				i, item_types[S_BUYTYPE(shop, i)],
...
void sedit_shop_flags_menu(DescriptorData *d) {
	...
	for(i = 0; i < NUM_SHOP_FLAGS; i++) {
		sprintf(buf, "`g%2d`n) %-20.20s   %s", i+1, shop_bits[i],
				!(++count % 2) ? "\r\n" : "");
		send_to_char(buf, d->character);
	}
	send_to_char("\r\nCurrent Shop Flags : `c", d->character);
...
void sedit_no_trade_menu(DescriptorData *d) {
	...
	for(i = 0; i < NUM_TRADERS; i++) {
		sprintf(buf, "`g%2d`n) %-20.20s   %s", i+1, trade_letters[i],
				!(++count % 2) ? "\r\n" : "");
		send_to_char(buf, d->character);
	}
	send_to_char("\r\nCurrently won't trade with: `c", d->character);
...
void sedit_types_menu(DescriptorData *d) {
	...
	for(i=0;  i < NUM_ITEM_TYPES; i++) {
		sprintf(buf, "`g%2d`n) `c%-20s  %s", i, item_types[i],
				!(++count % 3) ? "\r\n" : "");
		send_to_char(buf, d->character);
	}
	release_buffer(buf);
	send_to_char("`nEnter choice : ", d->character);
...
void sedit_disp_menu(DescriptorData *d) {
	...
	sprintf(buf,
				...
				"Enter Choice : ",
			OLC_NUM(d),
			S_KEEPER(shop) == NOBODY ? NOBODY : mob_index[S_KEEPER(shop)].vnum, 
			S_KEEPER(shop) == NOBODY ? "None" : SSData(((CharData *)mob_index[S_KEEPER(shop)].proto)->general.shortDesc),
...
/*-------------------------------------------------------------------*/
  case SEDIT_CONFIRM_SAVESTRING:
    ...
    case 'Y':
      ...
      mudlogf(NRM, LVL_BUILDER, TRUE, "OLC: %s edits shop %d", d->character->RealName(), OLC_NUM(d));
