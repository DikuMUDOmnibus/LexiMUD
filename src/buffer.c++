...
#define BUF_NUKE		(1 << 0)	//	Use memset() to clear after use
#define BUF_CHECK		(1 << 1)	//	Report how much is used
#define BUF_OVERBOOT	(1 << 2)	//	Reboot immediately on overflow
#define BUF_DETAIL		(1 << 3)	//	Output detail
#define BUF_VERBOSE		(1 << 4)	//	Output extremely detailed info

Flags	buffer_opt = BUF_NUKE;
...
#if BUFFER_THREADED
	SInt8		locked;			//	Don't touch this item, we're locked.
#endif
	SInt8		type;			//	What type of buffer are we?
...
SInt32						buffer_cache_stat[2] = { 0, 0 };
static SInt32				buffer_array[2] = { 0, 0 };
...
/* Private functions. */
static struct buf_data *new_buffer(size_t size, SInt32 type);
static struct buf_data *malloc_buffer(size_t size, SInt32 type);
...
static int get_magnitude(UInt32 number);
struct buf_data *find_free_buffer(size_t size);
ACMD(do_overflow);
ACMD(do_buffer);
#if BUFFER_THREADED
static void *buffer_thread(void *);
static void bufferlist_lock(const char *func, UInt16 line);
static void bufferlist_unlock(const char *func, UInt16 line);
static void buffer_lock(struct buf_data *buf, int type, const char *func, UInt16 line);
static void buffer_unlock(struct buf_data *buf, const char *func, UInt16 line);
#endif
#if BUFFER_MEMORY
...
#define set_buffer_head(x)	buffers.buf = (x)
#define get_memory_head()	(buffers.mem)
#define set_memory_head(x)	buffers.mem = (x)
...
static int get_magnitude(UInt32 y) {
	UInt16 number = buffer_hash(y);
...
void init_buffers(void) {
	...
	if (buf_thread != 0)	pthread_cancel(buf_thread);
	...
	set_buffer_head((buf_data **)calloc(buffer_array[BUF_MAX] + 1, sizeof(struct buf_data *)));
	set_memory_head((buf_data **)calloc(buffer_array[BUF_MAX] + 1, sizeof(struct buf_data *)));
...
static void free_old_buffers(void) {
	...
	for (magnitude = 0; magnitude <= buffer_array[BUF_MAX]; magnitude++) {
		for (freebuf = head[magnitude]; freebuf; freebuf = next_free) {
			...
			if (freebuf->type != BT_STACK)	continue;	//	We don't free persistent ones
 ...
 */
struct buf_data *detach_buffer(char *data, SInt8 type, const char *func, const int line_n)
{
...
static void free_buffer(struct buf_data *f) {
	...
	if (f->data)		really_free(f->data);
	else				log("BUF: SYSERR: free_buffer: Hey, no data in %p?", f);
	really_free(f);
}
 ...
 */
char *acquire_buffer(size_t size, int type, const char *varname, const char *who, UInt16 line)
{
 ...
 */
void show_buffers(CharData *ch, int buffer_type, int display_type)
{
  ...
  if (display_type == 1 || display_type == 2) {
    for (size = MIN_ALLOC; size <= MAX_ALLOC; size++) {
      SInt32 bytes, magnitude = get_magnitude(1 << size);
      for (i = 0, bytes = 0, disp = head[magnitude]; disp; disp = disp->next, i++)
	bytes += disp->size;
      log("%5d bytes (%2d): %5d items, %6d bytes, %5d overhead.", (1 << size), magnitude, i, bytes, sizeof(struct buf_data) * i);
    }
    return;
  }
  ...
  for (size = 0; size <= buffer_array[BUF_MAX]; size++)
	...
	continue;
      sprintf(buf, "%1d #%2d %5d bytes, Life: %5d, Type: %7s, Allocated: %s/%d.%s",
