...
ACMD(do_action) {
	...
	if ((!action->char_body_found) && (*buf2)) {
		...
		if (GET_POS(vict) < action->min_victim_position)
			...
			if (*buf2)  {
				act(action->char_body_found, 0, ch, (ObjData *)buf2, vict, TO_CHAR | TO_SLEEP);
				act(action->others_body_found, action->hide, ch, (ObjData *)buf2, vict, TO_NOTVICT);
				act(action->vict_body_found, action->hide, ch, (ObjData *)buf2, vict, TO_VICT);
			} else  {
